...
Для осуществления мягких проверок следует использовать класс ru.lanit.AssertsManager. 
...
В классе ru.lanit.CustomCommands, необходимо релизовать свою логику, например код ниже, будет перед осуществлением клика, прикладывать скриншот элемента, на который будет происходить нажатие.    
```java
...
```
Изменения логики формирования Page объектов, осуществляется в классе ru.lanit.CustomSelenidePageFactory. С помощью этого класса можно расширить/изменить логику инициализации элементов. 
...
```properties
stand= названия стенда для тестирования  в классе (ru.lanit.Stand) содержатся адреса стендов
